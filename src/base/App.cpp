      ...
      m_toneMap(false)
{
    ...
    m_commonCtrl.addToggle((S32*)&m_shadingMode, Renderer::ShadingMode_AmbientOcclusion, FW_KEY_F2,
                           "Ambient occlusion shading (F2)");
    m_commonCtrl.addToggle((S32*)&m_shadingMode, Renderer::ShadingMode_Whitted, FW_KEY_F3,
    ...
    else
    {
        ...
        timingResult res = m_renderer->rayTracePicture(m_rt.get(), m_rtImage.get(), m_cameraCtrl,
        ...
        result << cmd_args[3] << " " << m_results.scene_name << " " << m_results.state_name << " " << m_results.
            build_time << " " << m_results.trace_time << " " << m_results.rayCount << std::endl;
    ...
    case Action_TracePrimaryRays:
        {
            if (m_rt)
            {
                ...
                m_renderer->rayTracePicture(m_rt.get(), m_rtImage.get(), m_cameraCtrl,
                                            (Renderer::ShadingMode)m_shadingMode);
                m_RTTextureNeedsUpload = true;
...
void App::blitRttToScreen(GLContext* gl)
{
    ...
    //get currently bound texture so texture bind state can remain unchanged
